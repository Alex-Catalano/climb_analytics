#(venv) alex@Alexs-MacBook-Pro climb_analytics % python3 analyze_yolov8_climb_metrics_decay_v4.py climbing_clip_1.mp4 --out outputs --telemetry --telemetry-h264

#(venv) alex@Alexs-MacBook-Pro climb_analytics %  python3 analyze_yolov8_climb_metrics_decay_v4.py climbing_clip_1.mp4 --out outputs --telemetry --telemetry-h264 
#more accuracy for jumps


# analyze_yolov8_climb_metrics_decay_v4.py
# YOLOv8 Pose (Ultralytics) climbing analysis (v4.2):
# - proxy conversion (optional)
# - pose tracking (largest bbox)
# - ROBUST body-relative movement score (translation-invariant)
#     * median aggregation
#     * torso fallback when limbs go out of view
#     * returns NaN on low-quality frames (prevents spikes)
#     * EMA updates only on valid frames
#     * IMPORTANT FIX: "prime" prev_kpts so EMA doesn't stay at 0 forever
# - auto active-climb segmentation
# - micro-pause detection only during active climbing (ignores invalid frames)
# - telemetry annotated video
# - windowed "decay" CSV + reliability metrics
# - CLIMBER METRICS + USEFUL GRAPHS:
#     * pose quality + usable keypoints
#     * hip/shoulder/hand/foot heights (progress + stance changes)
#     * reach span + stance width (with spike rejection)
#     * robust root travel speed (teleport filtered)
#     * jerkiness + variability
#     * WINDOWED "sloppiness" and "stability" indices (decay-friendly)
#     * validity overlays so you don’t model tracker failures as fatigue
#
# Run:
#   python3 analyze_yolov8_climb_metrics_decay_v4.py climbing_clip_1.mp4 --out outputs --telemetry --telemetry-h264
# Optional accuracy bump:
#   python3 analyze_yolov8_climb_metrics_decay_v4.py climbing_clip_1.mp4 --out outputs --model yolov8s-pose.pt --telemetry --telemetry-h264
...
REACH_STANCE_JUMP_THR = 0.12     # reject single-frame span jumps above this (normalized units)
ROOT_SPEED_TELEPORT_THR = 1.2    # reject teleport speeds above this (normalized units / s)
MIN_VALID_FRAC_FOR_DECAY = 0.60  # windows below this are unreliable for decay modeling
...
TORSO_IDXS = np.array([5, 6, 7, 8, 11, 12], dtype=int)  # shoulders, elbows, hips
MIN_TORSO_KPTS = 4
...
def compute_decay_windows(df_active, window_s, step_s):
    ...
    while start + window_s <= t1 + 1e-6:
        ...
        if len(win) >= 3:
            ...
            # pause fraction only on valid movement frames
            if valid.any():
                pause_frac = float((win.loc[valid, "is_paused"] == True).mean())
            else:
                pause_frac = np.nan

            # micro-pause segments inside window (valid frames only)
            ...
            # Progress proxy: hip_y trend (negative slope => moving up)
    ...
    if len(out):
        # Sloppiness (higher = more jerky/variable/pausy)
        # Use z-scores so it scales across different clips.
        ...
        # Mask indices for low-valid windows (prevents modeling tracker failures)
...
def analyze(video_path: str, model, device: str, t_start: float | None, t_end: float | None):
    ...
    for frame in raw_frames(cap):
        ...
        if prev_kpts is None:
            prev_kpts = kpts
            score = np.nan
            n_used = int((kpts[:, 2] >= KPT_CONF_THR).sum())
            q = float(np.nanmean(kpts[:, 2]))
            ema = (1 - MOVEMENT_EMA_ALPHA) * ema
        else:
            score, n_used, q = movement_body_relative_robust(prev_kpts, kpts, conf_thr=KPT_CONF_THR)
        ...
        # Robust root speed: require confident hips + reject teleports
        spd = np.nan
        dt = float(max(1e-6, ANALYSIS_STRIDE / fps))
        lh_ok = float(kpts[IDX_L_HIP, 2]) >= KPT_CONF_THR
        rh_ok = float(kpts[IDX_R_HIP, 2]) >= KPT_CONF_THR
        ...
        # extra features
        hips_y = float(r[0])
        sh = mid_shoulder(kpts)
        sh_y = float(sh[0])

        lw = _safe_joint_y(kpts, IDX_L_WRIST)
        rw = _safe_joint_y(kpts, IDX_R_WRIST)
        handmin = np.nanmin([lw, rw]) if (np.isfinite(lw) or np.isfinite(rw)) else np.nan

        la = _safe_joint_y(kpts, IDX_L_ANKLE)
        ra = _safe_joint_y(kpts, IDX_R_ANKLE)
        footmin = np.nanmin([la, ra]) if (np.isfinite(la) or np.isfinite(ra)) else np.nan

        reach = _safe_dist(kpts, IDX_L_WRIST, IDX_R_WRIST)
        stance = _safe_dist(kpts, IDX_L_ANKLE, IDX_R_ANKLE)

        # Reject single-frame “confident wrong” span teleports
    ...
    # Derived metrics: jerk (EMA derivative) + rolling variability
    ...
    # Micro-pause segments inside active, valid movement frames only
...
def write_telemetry(video_path: str, out_path: str, model, device: str, panel_w: int = 320,
    ...
    while True:
        ...
        draw_panel(panel, lines, header="Climb Telemetry (YOLO Pose v4.2)")
...
def make_plots(out_dir: Path, stem: str, df: pd.DataFrame, df_active: pd.DataFrame, decay_df: pd.DataFrame):
    ...
    ax.set_title(f"{stem} movement v4.2 (active shaded)")
    ax.legend()
    save_plot(out_dir / f"{stem}_plot_movement.png", fig)

    # 2) Pose quality + usable keypoints
    fig, ax = plt.subplots()
    ...
    # 3) Hip/shoulder height (normalized y; lower is higher)
    fig, ax = plt.subplots()
    ...
    # 6) Root speed + jerk (teleport-filtered)
    fig, ax = plt.subplots()
    ...
    # 8) Active-only histogram of movement EMA
    if len(df_active) >= 5:
        fig, ax = plt.subplots()
    ...
    # 9) Decay windows: sloppiness + stability + validity
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Climbing analysis using YOLOv8 Pose (v4.2): robust movement, active segmentation, decay + climber metrics + useful plots."
    )
    ...
    # Save CSVs
    csv_path = out_dir / f"{stem}_yolo_pose_timeseries_v4.csv"
    csv_active_path = out_dir / f"{stem}_yolo_pose_active_only_v4.csv"
    decay_path = out_dir / f"{stem}_decay_windows_v4.csv"
    summary_path = out_dir / f"{stem}_summary_v4.csv"
    ...
    # Telemetry
    if args.telemetry:
        telemetry_raw = out_dir / f"{stem}_telemetry_yolo_v4.avi"
        ...
        if args.telemetry_h264:
            telemetry_h264 = out_dir / f"{stem}_telemetry_yolo_v4_h264.mp4"
