#(venv) alex@Alexs-MacBook-Pro climb_analytics % python3 analyze_yolov8_climb_consolidated_v7.py climbing_clip_1.mp4 --out outputs --telemetry --telemetry-h264

#(venv) alex@Alexs-MacBook-Pro climb_analytics % python3 analyze_yolov8_climb_consolidated_v7.py climbing_clip_1.mp4 --out outputs --quality max --telemetry --telemetry-h264

# analyze_yolov8_climb_consolidated_v7.py
# v7 = consolidated successor to v4/v5/v6:
# - tracker-aware target locking (track id + center continuity fallback)
# - external tracker config hook (defaults to repo-local ByteTrack tuning)
# - quality presets (fast/balanced/max) with overridable inference knobs
# - v6 sanity gating + torso-first movement + rich metrics/plots
#
# Goal: make decay modeling possible by turning pose failures into "missing data" instead of spikes.
#
# Run:
#   python3 analyze_yolov8_climb_consolidated_v7.py climbing_clip_1.mp4 --out outputs --telemetry --telemetry-h264
#
# More accuracy (slower):
#   python3 analyze_yolov8_climb_consolidated_v7.py climbing_clip_1.mp4 --out outputs --quality max --telemetry --telemetry-h264
...
# YOLO model
YOLO_MODEL_NAME = "yolov8s-pose.pt"  # balanced default
...
# Inference/tracking controls
INFER_CONF = 0.20
INFER_IOU = 0.55
INFER_IMGSZ = 832
INFER_AUGMENT = False
INFER_MAX_DET = 8
INFER_PERSON_ONLY = True
USE_TRACKING = True
TRACKER_CFG = str(Path(__file__).resolve().parent / "configs/trackers/bytetrack_climb.yaml")
TRACK_MAX_MISSES = 45
...
BONE_JUMP_FRAC = 0.70         # reject if bone length changes >70% between frames
JOINT_TELEPORT = 0.28         # reject if wrist/ankle moves >0.28 normalized in one step
ROOT_SPEED_TELEPORT = 3.5     # reject root speed above this (norm/s)
RELAXED_SANITY_MIN_POSE_QUALITY = 0.22
...
def apply_quality_preset(name: str):
    global YOLO_MODEL_NAME
    global ANALYSIS_STRIDE, TELEMETRY_STRIDE_SLOW, TELEMETRY_STRIDE_FAST
    global INFER_CONF, INFER_IOU, INFER_IMGSZ, INFER_AUGMENT
...
class TargetSelector:
    """
    Stable person selection:
    1) keep locked tracker ID when available
    2) fallback to nearest center to prior target
    3) fallback to largest bbox
    """
    def __init__(self, max_misses: int = 45):
    ...
    def choose(self, boxes: np.ndarray, track_ids: np.ndarray | None):
        ...
        if track_ids is not None and self.locked_track_id is not None:
            cand = np.where(track_ids == self.locked_track_id)[0]
            if len(cand) > 0:
                idx = int(cand[0])

        if idx is None and self.last_center is not None:
            d = np.linalg.norm(centers - self.last_center[None, :], axis=1)
            score = d / np.sqrt(np.maximum(areas, 1.0))
            idx = int(np.argmin(score))

        if idx is None:
            idx = int(np.argmax(areas))

        self.last_center = centers[idx].copy()
        self.misses = 0
...
def run_yolo_pose(model, frame_bgr, device: str, selector: TargetSelector | None, use_track: bool = True):
    """
    ...
    if INFER_PERSON_ONLY:
        common_kwargs["classes"] = [0]

    tracker_cfg = TRACKER_CFG if Path(TRACKER_CFG).exists() else "bytetrack.yaml"

    # Try tracking first for stable identity; fallback to predict.
    try:
        if use_track and USE_TRACKING:
            results = model.track(
                **common_kwargs,
                persist=True,
                tracker=tracker_cfg,
    ...
    track_ids = _to_numpy(getattr(r.boxes, "id", None))
    ...
    kpts_xyn = _to_numpy(kpts_xyn)
    kpts_conf = _to_numpy(kpts_conf)
    ...
    if track_ids is not None:
        track_ids = track_ids.astype(np.int64).reshape(-1)

    if selector is not None:
        idx = selector.choose(boxes, track_ids)
        if idx is None:
            selector.note_miss()
            return None, None, 0.0, False
    else:
...
def build_and_save_cropped_video(video_path: str, out_dir: Path, model, device: str):
    p = Path(video_path)
    crop_path = out_dir / f"{p.stem}_crop_safe_v7.mp4"
    if crop_path.exists():
    ...
    sampled = {}
    prev_smooth = None
    selector = TargetSelector(max_misses=TRACK_MAX_MISSES)
    ...
    while True:
        ...
        if frame_idx % CROP_STRIDE == 0:
            kpts, bb, _, ok_pose = run_yolo_pose(
                model,
                frame,
                device=device,
                selector=selector,
    ...
    if _have("ffmpeg"):
        qt_path = out_dir / f"{p.stem}_crop_safe_v7_h264.mp4"
        try:
...
def analyze(video_path: str, model, device: str, t_start: float | None, t_end: float | None):
    ...
    while True:
        ...
        kpts, bb, area_frac, ok_pose = run_yolo_pose(
            model,
            frame,
            device=device,
            selector=selector,
        ...
        # Compute movement score torso-first.
        score = np.nan
        kind = "none"
        ...
        if prev_kpts is not None:
            score_try, n_use, q_use, kind_try = movement_body_relative_torso_first(
                prev_kpts, kpts_s, conf_thr=KPT_CONF_THR
            )
            n = int(n_use)
            q = float(q_use)
            if ok_sanity and np.isfinite(score_try):
                score = float(score_try)
                kind = kind_try
            elif (not ok_sanity) and np.isfinite(score_try) and n_use >= MIN_GOOD_KPTS and q_use >= RELAXED_SANITY_MIN_POSE_QUALITY:
                score = float(score_try)
                kind = f"{kind_try}_relaxed"
                relaxed_used = True

        # Accept if:
        # - sanity pass with finite score, or
        # - relaxed fallback recovered a finite score, or
        # - torso-only warmup frame (used to prime prev pose)
        torso_good = int(((kpts_s[:, 2] >= KPT_CONF_THR) & np.isin(np.arange(17), TORSO_IDXS)).sum())
        is_valid = bool(
            (ok_sanity and (np.isfinite(score) or torso_good >= MIN_TORSO_KPTS))
            or (relaxed_used and np.isfinite(score))
        )
        ...
        if is_valid:
            ...
            # keep prior accepted pose anchor when frame is rejected
            ema = (1 - MOVEMENT_EMA_ALPHA) * ema
    ...
    while True:
        ...
        if has_pose:
            ...
            if prev_kpts is not None:
                ...
                n = int(n_use)
                q = float(q_use)
                if ok_s and np.isfinite(score_try):
                    move = float(score_try)
                    kind = kind_try
                elif (not ok_s) and np.isfinite(score_try) and n_use >= MIN_GOOD_KPTS and q_use >= RELAXED_SANITY_MIN_POSE_QUALITY:
                    move = float(score_try)
                    kind = f"{kind_try}_relaxed"
                    relaxed_used = True
            ...
            torso_good = int(((kpts_s[:, 2] >= KPT_CONF_THR) & np.isin(np.arange(17), TORSO_IDXS)).sum())
            valid_move = bool(
                (ok_s and (np.isfinite(move) or torso_good >= MIN_TORSO_KPTS))
                or (relaxed_used and np.isfinite(move))
            )

            if valid_move:
                if np.isfinite(move):
                    ema = MOVEMENT_EMA_ALPHA * float(move) + (1 - MOVEMENT_EMA_ALPHA) * ema
                else:
                    ema = (1 - MOVEMENT_EMA_ALPHA) * ema
                prev_kpts = kpts_s
            else:
        ...
        lines = [
            ...
            ("v7 (consolidated)", (200, 200, 200)),
        ]
        draw_panel(panel, lines, header="Climb Telemetry (YOLO Pose v7)")
...
def make_plots(out_dir: Path, stem: str, df: pd.DataFrame, df_active: pd.DataFrame, decay_df: pd.DataFrame):
    ...
    ax.set_title(f"{stem} movement v7 (active shaded)")
    ax.legend()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Climbing analysis using YOLOv8 Pose (v7): consolidated tracking, sanity gating, and decay metrics."
    )
    ...
    parser.add_argument("--model", default="", help="Override pose model path/name")
    ...
    parser.add_argument("--tracker", default=TRACKER_CFG, help="Tracker YAML path")
    parser.add_argument("--track-max-misses", type=int, default=TRACK_MAX_MISSES,
                        help="Frames to keep target lock with missing detections")
    parser.add_argument("--no-track", action="store_true", help="Disable model.track and use model.predict only")
    parser.add_argument("--augment", dest="augment", action="store_true",
                        help="Enable test-time augmentation (slower)")
    ...
    parser.add_argument(
        ...
        default="none",
        help="Convenience mode for annotated video output: none, avi, or h264"
    )
    ...
    apply_quality_preset(str(args.quality))
    ...
    if str(args.model).strip():
        YOLO_MODEL_NAME = str(args.model).strip()

    if args.conf is not None:
        INFER_CONF = float(args.conf)
    if args.iou is not None:
        INFER_IOU = float(args.iou)
    if args.imgsz is not None:
        INFER_IMGSZ = int(args.imgsz)
    if args.augment is not None:
        INFER_AUGMENT = bool(args.augment)

    TRACKER_CFG = str(args.tracker)
    TRACK_MAX_MISSES = int(args.track_max_misses)
    USE_TRACKING = (not bool(args.no_track))

    want_telemetry = bool(args.telemetry)
    want_telemetry_h264 = bool(args.telemetry_h264)
    if args.visuals == "avi":
        want_telemetry = True
    elif args.visuals == "h264":
        want_telemetry = True
        want_telemetry_h264 = True
    ...
    tracker_cfg = TRACKER_CFG if Path(TRACKER_CFG).exists() else "bytetrack.yaml"
    print(
        f"[config] quality={args.quality} model={YOLO_MODEL_NAME} device={device} "
        f"imgsz={INFER_IMGSZ} conf={INFER_CONF} iou={INFER_IOU} "
        f"augment={INFER_AUGMENT} tracking={USE_TRACKING} tracker={tracker_cfg} "
        f"visuals={args.visuals}"
    )
    ...
    csv_path = out_dir / f"{stem}_timeseries_v7.csv"
    csv_active_path = out_dir / f"{stem}_active_only_v7.csv"
    decay_path = out_dir / f"{stem}_decay_windows_v7.csv"
    summary_path = out_dir / f"{stem}_summary_v7.csv"
    ...
    # Telemetry
    if want_telemetry:
        telemetry_raw = out_dir / f"{stem}_telemetry_v7.avi"
        ...
        if want_telemetry_h264:
            telemetry_h264 = out_dir / f"{stem}_telemetry_v7_h264.mp4"
